...
function Client(server, nick, opt) {
    ...
    if (!self.opt.server || self.opt.server.trim().length == 0)
        self.emit('error', new Error("server is null or empty, this is invalid"));
    
    if (!self.opt.nick || self.opt.nick.trim().length == 0)
        self.emit('error', new Error("nick is null or empty, this is invalid"));
    
    // fail if username has a space in it
    if (self.opt.nick.match(/ +/g))
        self.emit('error', new Error("username contains a space, this is invalid"));
    
    if (self.opt.autoConnect)
        self.connect();

		//allow unlimited listeners for now, I'll try to go through and get the client be better about cleaning up connections later
    ...
    self.addListener("raw", function (message) { // {{{
        switch (message.command) {
            ...
            case "rpl_lUserMe":
            case "rpl_lUserUnknown": //check to see if we should care about this: http://www.mirc.net/raws/?view=253
                // random welcome messages, dumping for now
                break;
            case "rpl_localUsers":
            case "rpl_globalUsers":
            case "rpl_highestConnectionCount":
            ...
            default:
                if (message.commandType == 'error') {
                    ...
                    self.emit('error', new Error(message));
                		...
                		//check for quit message, not sure if this is a good way to handle this.
                		if (message.args[0] && message.args[0].matches(/Closing Link.* \(Client Quit\)/i))
                			break;
...
Client.prototype.connect = function (retryCount) { // {{{
    ...
    var connectionCallback = function() { 
    	...
    	self.conn.removeListener("connect", connectionCallback);
    };
    ...
    if (self.opt.secure) {
      ...
      if (typeof self.opt.secure !== 'object')
        creds = {};
      self.conn = tls.connect(self.opt.port, self.opt.server, creds, 
        connectionCallback);
    } else {
      self.conn = net.createConnection(self.opt.port, self.opt.server);
      self.conn.addListener("connect", connectionCallback);
    }
    ...
    self.conn.addListener("close", function() {
        ...
        setTimeout(function() { self.connect( retryCount + 1 ); }, self.opt.retryDelay);
    });
}; // }}}

Client.prototype.postConnectionTasks = function () { // {{{
  this.conn.requestedDisconnect = false;
  this.conn.setTimeout(0);
  this.conn.setEncoding('utf8');
  
  if (this.opt.secure && !this.conn.authorized) {
...
Client.prototype.disconnect = function( message ) { // {{{ 
    ...
    if (this.conn.connected) {
      self.cleanupConnection(message);
    } else {
...
Client.prototype.cleanupConnection = function(message) { // {{{
